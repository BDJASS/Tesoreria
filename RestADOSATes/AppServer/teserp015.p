...
PROCEDURE GetRepCob:
    ...
    ASSIGN 
        l-meses = "Enero,Febrero,Marzo,Abril,Mayo,Junio,Julio,Agosto," +
    ...
    DO l-i = 1 TO NUM-ENTRIES(l-idMC):
        CREATE w-mc.
        ASSIGN 
            w-mc.Id-MC = INTEGER(ENTRY(l-i,l-IdMC))
            w-mc.tot15 = 0
            w-mc.tot10 = 0
            w-mc.tot0  = 0.
    END.
    ...
    FOR EACH MovCliente WHERE MovCliente.FecReg >= l-fecini AND
        ...
        IF FIRST-OF(MovCliente.FecReg) THEN 
        DO:
        ...
        IF NOT AVAILABLE Acuse OR
            AVAILABLE Acuse AND Acuse.Estatus = 4 THEN 
        DO:
            FIND TipoPago WHERE TipoPago.Id-TP = MovCliente.Id-MC NO-LOCK NO-ERROR.

            IF MovCLiente.Id-mc <> 90 AND movcliente.id-mc > 62 THEN 
            DO:
                ...
                DO:
                    ...
                    DO:
                        ASSIGN 
                            l-tot0 = l-tot0  + (MovCliente.Importe * -1).
                    IF AVAILABLE w-mc THEN 
                    ...
                    DO:
                        ASSIGN 
                            l-tot15 = l-tot15 + (MovCliente.Importe * -1).
                        IF AVAILABLE w-mc THEN
                            ASSIGN w-mc.Tot15 = w-mc.Tot15 + ((MovCliente.Importe * -1) / 1.15).
                ...
                FOR EACH DistIVA WHERE DistIVA.Id-Factura = MovCliente.RefSaldo
                    ...
                    END.
                    ASSIGN 
                        l-count = (IF (ACCUM COUNT b-distiva.id-factura) > 1 
                    ...
                    DO:
                        ASSIGN 
                            l-tot10 = l-tot10 + ((MovCliente.Importe * -1) *
                     ( IF (DistIVA.Participacion > 100) OR NOT l-count THEN 1 
                              ELSE (DistIva.Participacion / 100)) ).
                        IF AVAILABLE w-mc THEN
            ...
            END.
        END.
        IF LAST-OF(MovCliente.FecReg) THEN 
        DO:
            CREATE ttRepCob.
            ASSIGN
    ...
    FIND FIRST w-mc WHERE w-mc.Id-MC = 9999 EXCLUSIVE-LOCK NO-ERROR.
    ASSIGN 
        w-mc.Tot15 = (ACCUM TOTAL l-tot15) - ((ACCUM TOTAL l-tot15) / 1.15)
  ...
  // DISPLAY STREAM s-salida
    ...
    CREATE  ttDetPago.
    ASSIGN 
        ttDetPago.Descripcion = "Neto de Descuentos"
    ...
    CREATE  ttDetPago.
    ASSIGN 
        ttDetPago.Descripcion = "IVA de Descuentos"
    ...
    FOR EACH w-mc WHERE w-mc.Id-Mc <> 9999 NO-LOCK:
        ...
        CREATE  ttDetPago.
        ASSIGN 
            ttDetPago.Descripcion = TabMC.Descr 
