...
DEFINE OUTPUT PARAMETER Mensaje AS CHARACTER NO-UNDO.
    ...
    RUN CancelaCFD(INPUT CPago.Id-CPago, INPUT IdUser). //Se habilita para producci�n    
    ...
    IF CPago.Id-Cliente = 3 THEN DO:
        ...
        v-recep = '<Receptor rfc="' + v-rfcCte + '" nombre="' + v-razon + '" >'.
        
        v-enca = '<ImprimeEncabezado '.
    ...
    ELSE DO:
        ...
        v-recep = '<Receptor rfc="' + v-rfcCte + '" UsoCFDI="P01" nombre="' + v-razon + '" >'.
    ...
    */
    ...
    v-pago = '<Pago FechaPago="' + STRING(YEAR(CPago.FecPag),"9999") + '-' + STRING(MONTH(CPago.FecPag),"99") + '-' + STRING(DAY(CPago.FecPag),"99") + 
                   'T00:00:00" ' +
                   'FormaDePagoP="' + CPago.FormaDePago + '" MonedaP="MXN" Monto="' + TRIM(STRING(l-Suma)) + '"> '.
    ...
    FOR EACH DocAcuse OF Acuse NO-LOCK:
        ...
        v-pago = v-pago + 'Folio="' + ENTRY(3,Factura.Folioe) + '" MonedaDR="MXN" MetodoDePagoDR="PPD" NumParcialidad="' + TRIM(STRING(DocAcuse.NParcialidad)) + '" ' +
                          'ImpSaldoAnt="' + TRIM(STRING(DocAcuse.SaldoAnt)) + '" ImpSaldoInsoluto="' + TRIM(STRING(DocAcuse.SaldoFin)) + '" ' +
                          'ImpPagado="' + TRIM(STRING(DocAcuse.ImpPago)) + '"/>'.
    END.
    v-pago = v-pago + "</Pago>".
    ...
    v-comprobante = v-comprobante + '<Emisor rfc="AOF870529IU7" nombre="Abastecedora de Oficinas, S.A. de C.V.">'.
    ...
    END.
    ELSE DO:
        ASSIGN Mensaje = " No se pudo conectar al Web Service".
...
IF v-estatus = 'A' THEN DO:
    ...
    IF vlconnect THEN DO:
        ...
        IF v-valores[1] = "ERROR" THEN DO:    
            ...
            ASSIGN l-Mensaje = "Ocurrio un error al intentar cancelar el CFDI CPago " + v-valores[3].
        ...
        ELSE DO:
            ...
            ASSIGN l-Mensaje = "El CFDI CPago se Cancelo con exito, Folio: " + v-valores[1] + "," + v-valores[2] + "," + v-valores[3] + "," + v-valores[4].            
            
        END.
    END.
    ELSE DO:
        
        ASSIGN l-Mensaje = "Ocurrio un error. No se puede conectar con el servicio...".
        
    END.        
END.
ELSE IF v-estatus = 'C' THEN DO:
        
        ASSIGN l-Mensaje = "El CFDI CPago ya esta cancelado".
        
     END.    
     ELSE DO:
        
        ASSIGN l-Mensaje = "El CFDI CPago no a sido generada".
...
PROCEDURE IntentaCFD.
   ...
   DO WHILE l-Intent <= l-MaxInt:
      ...
      IF v-valores[1] <> "ERROR" AND v-valores[1] BEGINS 'A' THEN DO: 
          ...
          DO TRANSACTION:
              ASSIGN CPago.Folioe = v-valores[1] + ',' + v-serie + ',' + v-folio
      ...
      ELSE DO:
          ...
          ELSE DO:
             ...
             EXPORT v-respuesta.
             OUTPUT CLOSE.
             ASSIGN l-Mensaje = "Ocurrio un error al generar cfdi de comprobante de pago".
             LEAVE.
          END.          
      END.
   END.
...
PROCEDURE EnviaCorreo.
    ...
    FIND CPago WHERE CPago.Id-CPago = l-CPago NO-LOCK NO-ERROR.
    FIND Usuario WHERE Usuario.Id-User = USERID("dictdb") NO-LOCK NO-ERROR.
    ASSIGN
        ...
        l-Contenido = "<html><head>" +
                      ...
                      "USUARIO: <b>" + USERID("dictdb") + " " + Usuario.Nom-Usuario + "</b><br/><br/>" +
                      "<o:p></o:p></span></p>" +
