...
DEFINE TEMP-TABLE ttCorteSuc
    ...
    FIELD suc           AS CHAR 
    FIELD Efevo         AS DECIMAL FORMAT ">>>,>>>,>>>,>>9.99"
...
PROCEDURE GetDashBoard:
    ...
    ************************************************************* */
    ...
    FOR EACH tt-MovCaja WHERE tt-MovCaja.fecoper = l-Fecha 
        ...
        AND tt-MovCaja.tipoventa <> 3 
        NO-LOCK BREAK BY tt-MovCaja.Sucursal:    
                              
           /*
        IF tt-MovCaja.TipoVenta < 5 OR tt-MovCaja.TipoVenta = 7 THEN
            FOR EACH DetMovC WHERE DetMovC.Id-caja = tt-MovCaja.Id-Caja AND
                DetMovC.Folio = tt-MovCaja.Folio NO-LOCK:
        ...
        /* Procesamiento para movimientos tipo 6 con FolioAut = 0 */
        /*   IF tt-MovCaja.TipoVenta = 6 AND tt-MovCaja.FolioAut = 0 THEN 
           DO:
               FOR EACH b-Mov WHERE b-Mov.turno = tt-MovCaja.Turno AND
                   b-Mov.id-caja = tt-MovCaja.Id-caja AND
                   b-Mov.FecOper = tt-MovCaja.FecOper AND
                   b-Mov.Folio < tt-MovCaja.Folio NO-LOCK:
                   ...
                   IF b-Mov.TipoVenta = 6 AND b-Mov.FolioAut = 0 THEN
                       ASSIGN Efvo = Efvo - b-Mov.TotVenta.                
               END.   
           END. */
        /* Fin de procesamiento para movimientos tipo 6 */      
